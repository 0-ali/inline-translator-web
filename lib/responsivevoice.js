...
if ("undefined" != typeof responsiveVoice) console.log("ResponsiveVoice already loaded"),
    ...
    a.sleepFor = function (sleepDuration) {
        var now = new Date().getTime();
        while (new Date().getTime() < now + sleepDuration) { /* do nothing */ }
    };
    a.sleepThenAct = function (t) { a.sleepFor(t); return true; }
    a.wait = function () {
        while (true) {
    ...
    a.speak = function (b, c, f) {
        if (a.iOS9 && !a.iOS9_initialized) console.log("Initializing ios9"),
            setTimeout(function () {
                a.speak(b, c, f)
            }
                , 100),
            ...
            a.isPlaying() && (console.log("waiting event until finish"), a.wait());
            a.fallbackMode && 0 < a.fallback_audiopool.length && a.clearFallbackPool();
            b = b.replace(/[\"\`]/gm, "'"); a.msgparameters = f || {}; a.msgtext = b; a.msgvoicename = c; a.onstartFired = !1; var h = []; if (b.length > a.CHARACTER_LIMIT) {
                for (var e = b; e.length > a.CHARACTER_LIMIT;) {
                    var g = e.search(/[:!?.;]+/), d = ""; if (-1 == g || g >= a.CHARACTER_LIMIT) g = e.search(/[,]+/); -1 == g && -1 == e.search(" ") && (g = 99); if (-1 == g || g >= a.CHARACTER_LIMIT) for (var k =
                        e.split(" "), g = 0;
            ...
            else if (e.systemvoice = a.getMatchedVoice(g), e.collectionvoice = {}
                , null == e.systemvoice) {
            ...
            for (g = 0;
                ...
                g++)if (a.fallbackMode) {
                    ...
                    var d = a.selectBest([e.collectionvoice.pitch, e.systemvoice.pitch, 1]),
                        k = a.selectBest([a.iOS9 ? 1 : null, e.collectionvoice.rate, e.systemvoice.rate, 1]), l = a.selectBest([e.collectionvoice.volume, e.systemvoice.volume, 1]);
                    ...
                    d.voice = e.systemvoice,
                    d.voiceURI = e.systemvoice.voiceURI,
                    d.volume = a.selectBest([e.collectionvoice.volume, e.systemvoice.volume, 1]),
                    d.rate = a.selectBest([a.iOS9 ? 1 : null, e.collectionvoice.rate, e.systemvoice.rate, 1]),
                    d.pitch = a.selectBest([e.collectionvoice.pitch, e.systemvoice.pitch, 1]),
                    d.text = h[g],
                    d.lang = a.selectBest([e.collectionvoice.lang, e.systemvoice.lang]),
                    d.rvIndex = g,
                    d.rvTotal = h.length,
                    0 == g && (d.onstart = a.speech_onstart),
                    a.msgparameters.onendcalled = !1,
                    null != f ? (g < h.length - 1 && 1 < h.length ? (d.onend = a.onPartEnd, d.hasOwnProperty("addEventListener") && d.addEventListener("end", a.onPartEnd)) : (d.onend = a.speech_onend, d.hasOwnProperty("addEventListener") && d.addEventListener("end", a.speech_onend)), d.onerror = f.onerror || function (a) {
    ...
    a.mapRVs = function () {
        for (var b = 0;
            ...
            b++)for (var c = a.responsivevoices[b], f = 0;
                f < c.voiceIDs.length;
                f++) {
                ...
                if (1 != h.fallbackvoice) {
                    ...
                    if (null != e) {
                        c.mappedProfile = {
                            systemvoice: e, collectionvoice: h
                        }
                            ;
                        break
                    }
    ...
    a.getSystemVoice = function (b) {
        ...
        for (var c = 0;
            c < a.systemvoices.length;
            c++)if (a.systemvoices[c].name == b) return a.systemvoices[c];
        return null
    }
        ;
    a.getResponsiveVoice = function (b) {
        for (var c = 0;
            c < a.responsivevoices.length;
            c++)if (a.responsivevoices[c].name == b) return a.responsivevoices[c];
        return null
    }
        ;
    a.Dispatch = function (b) {
        if (a.hasOwnProperty(b + "_callbacks") && null != a[b + "_callbacks"] && 0 < a[b + "_callbacks"].length) {
            for (var c = a[b + "_callbacks"], f = 0;
                f < c.length;
                f++)c[f]();
            return !0
        }
